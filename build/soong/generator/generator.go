...
func init() {
	android.RegisterModuleType("liquid_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = liquidExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = liquidExpandVariables(ctx, dep_file)
	...
	cmd := liquidExpandVariables(ctx, String(g.properties.Cmd))
